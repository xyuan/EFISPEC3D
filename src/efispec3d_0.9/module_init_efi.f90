...
!>@return mod_global_variables::cg_prefix : prefix of input and output files
!***********************************************************************************************************************************************************************************
...
!***********************************************************************************************************************************************************************************
   ...
   !->cpu0 gather all the name of the other cpu
      ...
      if (ig_myrank == 0) then
         ...
         open(unit=IG_LST_UNIT,file=trim(cg_prefix)//".lst")
         write(IG_LST_UNIT,'(a)') "************************************************************"
...
!***********************************************************************************************************************************************************************************
      ...
      allocate(rg_gll_coordinate(IG_NDOF,ig_ngll_total),stat=nas)
      if (nas /= 0) then
         write(info,'(a)') "error in subroutine init_gll_nodes_coordinates while allocating rg_gll_coordinate"
         call error_stop(info)
      endif
   
      rg_gll_coordinate(:,:) = 0.0 
      rg_gll_coordinate(:,:) = 0.0
      rg_gll_coordinate(:,:) = 0.0
...
!***********************************************************************************************************************************************************************************
      ...
      if (lg_boundary_absorption .and. ig_nquad_parax > 0) then
         ...
         allocate(rg_quadp_gll_jaco_det(IG_NGLL,IG_NGLL,ig_nquad_parax),stat=nas)
         if (nas /= 0) then
            write(*,*) "***error in jacobi while allocating rg_quadp_gll_jaco_det. cpu ",ig_myrank
            call mpi_abort(mpi_comm_world,100,ios)
            stop
         endif
   
         allocate(rg_quadp_gll_normal(3,IG_NGLL,IG_NGLL,ig_nquad_parax),stat=nas)
         if (nas /= 0) then
            write(*,*) "***error in jacobi while allocating rg_quadp_gll_normal. cpu ",ig_myrank
            call mpi_abort(mpi_comm_world,100,ios)
            stop
         endif
         ...
         do iquad = 1,ig_nquad_parax
            do k = 1,IG_NGLL
               do l = 1,IG_NGLL
                  ...
                  rg_quadp_gll_jaco_det(l,k,iquad) = 0.0e0
   
                  rg_quadp_gll_normal(1,l,k,iquad) = 0.0e0
                  rg_quadp_gll_normal(2,l,k,iquad) = 0.0e0
                  rg_quadp_gll_normal(3,l,k,iquad) = 0.0e0
